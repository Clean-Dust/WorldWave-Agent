...
| **Coding Engine** | `coding/` (PM 0.13.0-endpoint) | Hard Arena v2 + SB1 baseline, index facade, large-repo `--real`, MCP/ACP coding tools. Persistent memory + tool harness layer alongside other coding CLIs — **does not claim exceeds Claude Code/Codex** unless F2b h2h scored a win. See `docs/coding-north-star.md`. Run: `python scripts/coding_arena.py --smoke`. |
